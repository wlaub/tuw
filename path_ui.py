...
#        self.window['cluster_rooms'].set_marked_items([0])
    ...
    def update_cluster_run_selection(self):
        ...
        for idx in sel_idx:
            ...
            points = [(x.xpos, x.ypos) for x in run.states if x.collection_flags.value!=0]
            for x,y in points:
                circle(x,y,4)
...
#            return
        ...
        for x in self.export_runs:
            for room in x.run.rooms:
                runmap[room].append(x.run)

        try:
            start_time = time.time()


            for room, runs in runmap.items():
                def _filter(x):
                    return x.room == room
                ...
                plotter.finalize()
                plotter.render(f'render_{room}.png', show=False)
